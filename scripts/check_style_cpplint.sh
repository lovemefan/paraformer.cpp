...
paraformer_dir=$(cd $cur_dir/.. && pwd)

build_dir=$paraformer_dir/build
mkdir -p $build_dir
...
function check_style() {
  python3 $cpplint_src --root src/csrc $1 || abort $1
}
...
function do_check() {
  case "$1" in
    ...
    2)
      ...
      files=$(find $paraformer_dir/src/csrc -name "*.h" -o -name "*.cpp")
      ;;
    *)
...
echo "workdir: $paraformer_dir/src/csrc"
cd $paraformer_dir/src/csrc
